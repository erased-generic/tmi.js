...
class Client extends ClientBase {
	...
	say(channel, message, tags) {
		...
		const fullNonce = `tmi.js_${nonce}`;
		tags = { ...tags, 'client-nonce': fullNonce };
		const badNotices = [
			'unrecognized_cmd',
		...
		return this._sendMessage({ channel, message, tags }, (resolve, reject) => {
			// The message is assumed to be sent and recieved by Twitch. The "client-nonce" tag can be used to
			// detect a response on a USERNOTICE command, but it's not yet implemented here.
			const cleanup = () => {
				clearTimeout(timeout);
				this.removeListener('userstate', onMessage);
				this.removeListener('notice', onNotice);
			};
			const timeout = setTimeout(() => {
				cleanup();
				reject(new Error(`Timed out waiting for 'USERSTATE'`));
			}, timeoutMs);
			const onMessage = (message) => {
				if (message.tags['client-nonce'] !== fullNonce) {
					return;
				}
				cleanup();
				resolve([channel, message]);
			};
			const onNotice = (ch, msgid, message) => {
				if (ch !== channel || !badNotices.includes(msgid)) {
					return;
				}
				cleanup();
				reject(new Error(`Received NOTICE '${msgid}' while waiting for 'USERSTATE'`));
			};
			this.on('userstate', onMessage);
